...
(async () => {
    ...
    while (true) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            Upils.logger(`[${extensionId}] Starting ping for Account #${i + 1}, with proxy`);
            ...
            if (!lastExecution[token] || now - lastExecution[token] >= 24 * 60 * 60 * 1000) {
                ...
                await Upils.checkAndClaimTask(extensionId, token);
                await Upils.dailyCheckin(extensionId, token);
